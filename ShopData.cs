...
namespace MarketTown
{
    ...
    {
        public static void GenerateShop(string shopId, Vector2 tile, Chest chest)
        {
            ShopsJsonData.Clear();
                ...
                if (shop.Key == shopId)
                {
                    ...
                    while (i <= shopLists[shopId].Count && i <= 9 && tried <= 18)
                    {
                        ...
                        if (randomItemId.StartsWith("(W)"))
                        {
                            ...
                            salePrice = salePrice * salePrice / 30;
                        }
                        ...
                        if (!initShopStockList.Contains(randomItemId) && !shop.Value.Items.Contains(newItem))
                        {
                            shop.Value.Items.Add(newItem);
                            initShopStockList.Add(randomItemId);
                            i++;

                            var itemTest = ItemRegistry.Create<Item>(randomItemId);
                            chest.Items.Add(itemTest);
                        }
                    }
                }
            }
